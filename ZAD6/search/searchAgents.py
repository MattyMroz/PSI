...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        # ZAD 5
        # Stan = (pozycja, tuple nieodwiedzonych narożników)
        # Na początku wszystkie narożniki są nieodwiedzone
    ...
    def isGoalState(self, state: Any):
        ...
        "*** YOUR CODE HERE ***"
        # ZAD 5
        # Cel osiągnięty gdy wszystkie narożniki odwiedzone
        # (czyli tuple nieodwiedzonych jest puste)
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            # ZAD 5
            # Pobieramy aktualną pozycję i nieodwiedzone narożniki
            x, y = state[0]
            ...
            # Sprawdzamy czy nie wchodzi w ścianę
            if not self.walls[nextx][nexty]:
                nextPosition = (nextx, nexty)

                # Jeśli weszliśmy na narożnik - usuń go z nieodwiedzonych
                if nextPosition in unvisitedCorners:
                    # Tworzymy nową listę bez tego narożnika
                    newUnvisited = tuple(
                        c for c in unvisitedCorners if c != nextPosition)
                else:
                    newUnvisited = unvisitedCorners

                # Nowy stan to nowa pozycja + zaktualizowana lista narożników nieodwiedzonych
                nextState = (nextPosition, newUnvisited)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Jeśli wszystkie odwiedzone - koszt = 0
    if len(unvisitedCorners) == 0:
        return 0

    # Heurystyka: zachłannie idź do najbliższego nieodwiedzonego narożnika
    # Suma odległości w kolejności zachłannej
    currentPos = position
    remaining = list(unvisitedCorners)
    totalDistance = 0

    while remaining:
        # Znajdujemy najbliższy narożnik
        distances = []
        for corner in remaining:
            dist = abs(currentPos[0] - corner[0]) + \
                abs(currentPos[1] - corner[1])
            distances.append((dist, corner))

        # Wybieramy najbliższy
        minDist, closestCorner = min(distances, key=lambda x: x[0])
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    "*** YOUR CODE HERE ***"
    # ZAD 7 Problemu Komiwojażera
    # Pobieramy liste pozycji jedzenia
    foodList = foodGrid.asList()

    # Jesli brak jedzenia - cel osiagniety
    if len(foodList) == 0:
        return 0

    # Heurystyka: odległość Max Manhattan Distance do najdalszej kropki
    maxDistance = 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # ZAD 8
        # Używamy BFS aby znaleźć najbliższe jedzenie
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        # ZAD 8
        # Cel = dowolna pozycja z jedzeniem
